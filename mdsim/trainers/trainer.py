...
class Trainer(ABC):
    ...
    def __init__(
        ...
        if timestamp_id is None:
            ...
            if identifier:
                self.timestamp_id = f"{identifier}"
            else:
        ...
        if identifier:
            expname = f"{identifier}"
        else:
            expname = f"{expname}_{self.timestamp_id}"
        ...
        self.load()
        self.evaluator = Evaluator(task=name, no_energy=no_energy, no_force=not self.config["model_attributes"].get("regress_forces", True))
    ...
    def load_datasets(self):
        ...
        if self.config.get("dataset", None):
            self.train_dataset = registry.get_dataset_class(
                ...
                self.config["dataset"],
                percentages=self.config["dataset"].get("percentages", None),
            )
        ...
        if self.freeze_rep:
            if self.config["model"] == "gemnet_t":
                for name, param in self.model.named_parameters():
                    if gemnet_freeze_amount_to_fn[self.freeze_amount](name):
                        param.requires_grad = False
                    else:
                        param.requires_grad = True
            else:
                for name, param in self.model.named_parameters():
                    if "interactions" in name or "embedding" in name:
                        if "interactions.5" not in name:
                            param.requires_grad = False
        if self.reset_head:
            if self.config["model"] == "gemnet_t":
    ...
    def save(
        ...
        if (force_save or not self.is_debug) and distutils.is_master():
            if training_state:
                save_checkpoint(
                    {
                        "epoch": self.epoch,
            ...
            if self.normalizers is not None:
                if not self.no_energy:
                    out["energy"] = self.normalizers["target"].denorm(
                        out["energy"],
                        mean_energy_per_system=batch_list[0].energy_mean
                        if "energy_mean" in batch_list[0]
                        else None,
                        std_energy_per_system=batch_list[0].energy_std
                        if "energy_std" in batch_list
                        else None,
                    )
    ...
    @torch.no_grad()
    def validate(self, split="val", disable_tqdm=False, max_points=None, return_per_structure_errors=False):
        ...
        for i, batch in tqdm(
            ...
            if return_per_structure_errors:
                s_mae = metrics['forces_mae']['total'] - previous_force_mae_total
                previous_force_mae_total += s_mae 
                force_maes.append(s_mae)
                force_norms.append(batch[0].force.norm(dim=-1).mean().cpu().numpy())
        ...
        if return_per_structure_errors:
            return metrics, force_maes, force_norms
    ...
    def _compute_loss(self, out, batch_list):
        ...
        if not self.no_energy:
            ...
            if self.normalizer.get("normalize_labels", False):
                energy_target = self.normalizers["target"].norm(
                    energy_target,
                    mean_energy_per_system=batch_list[0].energy_mean
                    if "energy_mean" in batch_list[0]
                    else None,
                    std_energy_per_system=batch_list[0].energy_std
                    if "energy_std" in batch_list[0]
                    else None,
                )
    ...
    def _compute_metrics(self, out, batch_list, evaluator, metrics={}):
        ...
        if self.config["task"].get("eval_on_free_atoms", True) and self.config["model_attributes"].get("regress_forces", True):
