...
class Evaluator:
    ...
    def __init__(self, task=None, no_energy=False, no_force=False):
        assert task in ["s2ef"]
        self.task = task
        ...
        if self.no_force:
            if 'forces' in self.task_attributes[self.task]:
                self.task_attributes[self.task].remove("forces")
                self.task_attributes[self.task].remove("natoms")
                self.task_metrics[self.task].remove("forces_rmse")
                self.task_metrics[self.task].remove("forces_mae")
                self.task_metrics[self.task].remove("forces_cos")
                self.task_metrics[self.task].remove("forces_magnitude")
                self.task_primary_metric[self.task] = "energy_mae"    
