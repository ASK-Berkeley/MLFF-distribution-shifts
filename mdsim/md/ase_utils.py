...
class OCPCalculator(Calculator):
    ...
    def __init__(self, config_yml=None, checkpoint=None, 
                 test_data_src=None, energy_units_to_eV=1., molecule=None, trainer_obj=None, use_sgdml_for_energy=False):
        """
        ...
        self.use_sgdml_for_energy = use_sgdml_for_energy
        if self.use_sgdml_for_energy:
            model = np.load('bucky_108.npz', allow_pickle=True)
            self.sgdml_predictor = GDMLPredict(model, use_torch=True)
    ...
    def sgdml_energy(self, atoms):
        """
        Calculate energy using SGDML model.
        """
        

        e, _ = self.sgdml_predictor.predict(atoms.get_positions())
        print('Using energy pred!')
        return e
    ...
    def calculate(self, atoms, properties, system_changes):
        ...
        if self.use_sgdml_for_energy:
            energy_pred = self.sgdml_energy(atoms)
        else:
            energy_pred = predictions["energy"].item() * self.energy_units_to_eV
