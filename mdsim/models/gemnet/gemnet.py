...
class GemNetT(torch.nn.Module):
    ...
    def __init__(
        ...
        in_context_prior: bool = False,
        n_hidden_prior_embedding: int = 2,
        n_atom_types: int = 83,
    ):
        super().__init__()
        ...
        # The prior forces are passed in as node features.
        if in_context_prior:
            assert emb_size_atom % 2 == 0, "Embedding size must be even for in-context prior"
            self.in_context_prior_embedding = self.get_mlp(
                3, int(emb_size_atom / 2), n_hidden_prior_embedding, activation
            )
            self.in_context_prior_embedding = torch.nn.Sequential(*self.in_context_prior_embedding)
            # Embedding block
            self.atom_emb = AtomEmbedding(int(emb_size_atom / 2), n_atom_types=n_atom_types)
        else:
            self.in_context_prior_embedding = None
            self.atom_emb = AtomEmbedding(emb_size_atom, n_atom_types=n_atom_types)
    ...
    def forward(self, data):
        ...
        if self.in_context_prior_embedding is not None:
            h_prior = self.in_context_prior_embedding(data.forces_classical)
            h = torch.cat([h, h_prior], dim=1)
