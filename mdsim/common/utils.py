...
def build_config(args, args_override):
    ...
    if args.molecule is not None:
        assert config['dataset']['name'] == 'md17' or config['dataset']['name'] == 'md17_classical' or config['dataset']['name'] == 'spice', 'only MD17 datasets admit specification of the molecule.'
...
def compose_data_cfg(data_cfg): 
    ...
    if dataset_name == 'md17' or dataset_name == 'md17-multi-classical' or dataset_name == 'spice':
        if type(data_cfg['molecule']) == list:
            data_cfg['src'] = [os.path.join(data_cfg['src'], molecule, data_cfg['size'], 'train') for molecule in data_cfg['molecule']]
            data_cfg['name'] = 'md17-multi'
        else:
            data_cfg['src'] = os.path.join(data_cfg['src'], data_cfg['molecule'])
            data_cfg['name'] = 'md17-' + data_cfg['molecule']
    if not type(data_cfg['src']) == list:
        ...
        norm_stats = np.load(os.path.join('/'.join(data_cfg['src'][0].split('/')[:-1]), 'metadata.npy'), allow_pickle=True).item() # dummy since not used
    ...
    if not data_cfg['normalize_labels']:
        ...
        if data_cfg.get('return_delta', False):
            logging.info("Normalizing labels for force deltas!")
            data_cfg['grad_target_mean'] = float(norm_stats['f_delta_mean'])
            data_cfg['grad_target_std'] = float(norm_stats['f_delta_std'])
