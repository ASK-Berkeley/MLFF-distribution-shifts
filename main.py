...
class Runner(submitit.helpers.Checkpointable):
    ...
    def __call__(self, config):
        ...
        try:
            ...
            if type(train_data_cfg['src']) == list:
                if config.get('ttt_val_dataset_path', None) is not None:
                    val_cfg = {'src': [config['ttt_val_dataset_path']]}
                else:
                    val_cfg = {'src': [os.path.join(os.path.dirname(dir), 'val') for dir in train_data_cfg['src']]}
                config['dataset'] = [train_data_cfg, val_cfg]
            elif config.get('ttt_val_dataset_path', None) is not None:
                config['dataset'] = [
                    train_data_cfg,
                    {'src': config['ttt_val_dataset_path']}
                ]
            else:
                config['dataset'] = [
                    train_data_cfg,
            ...
            if 'force_norm_cutoff' in train_data_cfg:
                config['dataset'][1]['force_norm_cutoff'] = train_data_cfg['force_norm_cutoff']
                config['dataset'][1]['force_norm_flip'] = train_data_cfg.get('force_norm_flip', False)
            ...
            self.trainer = registry.get_trainer_class(
                ...
                finetuning=config.get("finetuning", False),
                freeze_rep=config.get("freeze_rep", False),
                reset_head=config.get("reset_head", False),
                ttt_params=config.get("ttt_params", None),
                freeze_amount=config.get("freeze_amount", 'max'),
            )
