...
def eval_and_init(config):
    ...
    if config['sim_type'] == 'ocp':
        ...
        model_ckpt = str(Path(model_dir) / f'checkpoints/{config["model_name"]}')
        ...
        calculator = OCPCalculator(config_yml=model_config, checkpoint=model_ckpt, 
                                   ...
                                   energy_units_to_eV=1., molecule=config.get('sim_molecule', None))
        if config['no_evaluate']:
            test_metrics = {}
        else:
            test_metrics, force_maes, force_norms = calculator.trainer.validate('test', max_points=config["max_test_points"], return_per_structure_errors=True)
            np.savez(str(Path(model_dir) / 'force_maes.npy'), f_mae=force_maes, f_norm=force_norms)
...
def simulate(config, calculator, test_metrics):
    ...
    if config['atoms_file'] is not None:
        atoms = ase.io.read(config['atoms_file'])
        print('Start simulation from atoms file.')
    ...
    # set up simulator.
    integrator = getattr(md_integrator, config["integrator"])(atoms, **config["integrator_config"])
...
if __name__ == '__main__':       
    ...
    parser.add_argument("--dataset_src", type=str, default=None)
    ...
    if args.timestep is not None:
        config['integrator_config']['timestep'] = args.timestep
    config["model_name"] = args.model_name
    config["atoms_file"] = args.atoms_file
    if args.dataset_src is not None:
        config["dataset_src"] = args.dataset_src
